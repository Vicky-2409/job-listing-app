...
async function seedDatabase() {
  try {
    ...
    function extractExperienceRange(expString: string): {
      min: number;
      max: number;
    } {
      // Default values
    ...
    const transformedJobs = (sampleJobsRaw as RawJob[]).map((job) => {
    ...
    const validJobs = transformedJobs.filter((job) => {
      ...
      if (!isValid) {
        console.warn(
          `Skipping invalid job entry: ${job.title || "Unknown title"} at ${
